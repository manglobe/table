    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                lastSelectedCol: '',
                hotSettings: {
                  data: typeof self.propTable.tableData === 'string' ? JSON.parse(self.propTable.tableData) : self.propTable.tableData,
                  mergeCells: self.propTable.cellInfo === true ? self.propTable.cellInfo : JSON.parse(self.propTable.cellInfo),
                  colHeaders: true,
                  rowHeaders: true,
                  minCols: 1,
                  minRows: 1,
        ...
        methods: {
            changeCellType(type) {
                ...
                for(var i = self.firstSelectedRow; i <= self.lastSelectedRow; i ++) {
                    for(var j = self.firstSelectedCol; j <= self.lastSelectedCol; j ++) {
                        if(self.hotSettings.data[i][j] && !isNaN(parseFloat(self.hotSettings.data[i][j]))){
                            switch(type) {
                                case 0: //常规
                                  self.$set(self.hotSettings.data[i], j, self.formatNumber(self.selectedData[i][j], null)); 
                                  break;
                                case 1: //数字
                                  self.$set(self.hotSettings.data[i], j, self.formatNumber(self.selectedData[i][j], null));
                                  break;
                                case 2: //数字(保留两位小数)
                                  self.$set(self.hotSettings.data[i], j, self.formatNumber(self.selectedData[i][j], 2));
                                  break;
                                case 3: //百分比
                                  self.$set(self.hotSettings.data[i], j, self.formatPercentage(self.selectedData[i][j], null));
                                  break;
                                case 4: //百分比(保留两位小数)
                                  self.$set(self.hotSettings.data[i], j, self.formatPercentage(self.selectedData[i][j], 2));
                                  break;
                            } 
                        } 
                    }
                ...
                switch(formula) {
                    case 'add':
                        for(let i = self.firstSelectedRow; i <= self.lastSelectedRow; i ++) {
                            ...
                            for(let j = self.firstSelectedCol + 1; j <= self.lastSelectedCol; j ++) {
                                let value = self.selectedData[i][j] || 0
                                result = Calculation(result).add(value)
                            }
                            self.$set(self.hotSettings.data[i], self.lastSelectedCol + 1, result)
                        }                     
                        break
                    case 'sub':
                        for(let i = self.firstSelectedRow; i <= self.lastSelectedRow; i ++) {
                    ...
                    case 'average':
                        for(let i = self.firstSelectedRow; i <= self.lastSelectedRow; i ++) {
                            ...
                            self.$set(self.hotSettings.data[i], self.lastSelectedCol + 1, average)
                        }                     
                        break    
                }
            },
                ...
                switch(formula) {
                    case 'add':
                        for(let j = self.firstSelectedCol; j <= self.lastSelectedCol; j ++) {
                            ...
                            for(let i = self.firstSelectedRow + 1; i <= self.lastSelectedRow; i ++) {
                                let value = self.selectedData[i][j] || 0
                                result = Calculation(result).add(value)
                            }
                            self.$set(self.hotSettings.data[self.lastSelectedRow + 1], j, result)
                        }                     
                        break
                    case 'sub':
                        for(let j = self.firstSelectedCol; j <= self.lastSelectedCol; j ++) {
                    ...
                    case 'average':
                        for(let j = self.firstSelectedCol; j <= self.lastSelectedCol; j ++) {
                            ...
                            self.$set(self.hotSettings.data[self.lastSelectedRow + 1], j, average)
                        }                     
                        break    
                }
            },
            saveTable() {
              ...
              let params = {
                ...
                cellInfo: JSON.stringify(this.hot1.getInstance().mergeCells.mergedCellInfoCollection),
                ...
                contextMenu: {
                    items: {
                        ...
                        "mergeCells": {
                            name: function name() {
                              debugger
                              var sel = self.hot1.getSelected();
                              var info = self.hot1.mergeCells.mergedCellInfoCollection.getInfo(sel[0], sel[1]);
                              if (info) {
                                return '取消合并单元格';
                              }
                              return '合并单元格';
